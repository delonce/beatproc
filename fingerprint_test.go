...
func TestFingerprintHashes(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		if assert.NoError(t, err) {
			assert.Equal(t, testcase.fingerprint, event.Fields["custom_fingerprint"], "hash: %v", f.config.Hash.name)
		}
	}
}
...
func TestFingerprintFieldConcat(t *testing.T) {
	...
	if assert.NoError(t, err) {
		assert.Equal(t,
			...
			event.Fields["custom_fingerprint"])
	}
}
...
func TestFingerprintString(t *testing.T) {
	f := newTestFingerprint(t, "hex", "sha1")
	assert.Equal(t, "custom_fingerprint=[fields=message, hash=sha1, target=custom_fingerprint]", f.String())
}
